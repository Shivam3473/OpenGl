...
const char *vertexShaderSource = "#version 330 core\n"
                                 ...
                                 "uniform int u_ellipticalStartIndex;\n" // NEW: Pass the start of the Elliptical array
                                 "out vec4 particleColor;\n"
                                 "void main() {\n"
                                 "   gl_Position = transform * vec4(aPosMass.xyz, 1.0f);\n"
                                 "   if (gl_VertexID < u_numDiskParticles) {\n"
                                 "       particleColor = vec4(1.0f, 0.95f, 0.6f, 1.0f); \n"
                                 "   } else if (gl_VertexID >= u_ellipticalStartIndex) {\n"
                                 "       particleColor = vec4(1.0f, 1.0f, 1.0f, 1.0f); // Reddish-yellow Elliptical\n"
                                 "   } else {\n"
...
int main()
{
    ...
    // ---------------------------------------------------------
    const int NUM_PARTICLES = 110000;
    ...
    for (int i = 0; i < NUM_DISK_PARTICLES; ++i)
    {
        ...
        if (u < prob(r, z))
        {
            ...
            particles[i].velocity = glm::vec4(v_x-0.6f, v_y+1.2f, vz, 0.0f);
        }
        else
        {
            i--;
    ...
    for (int i = NUM_DISK_PARTICLES; i < NUM_PARTICLES; ++i)
    {
        ...
        particles[i].velocity = glm::vec4(vx + v_cx-0.6f, vy + v_cy+1.2f, vz + v_cz, 0.0f);
    };


    // ---------------------------------------------------------
// INITIALIZING THE ELLIPTICAL GALAXY (Pure Hernquist Sphere)
// ---------------------------------------------------------

const int NUM_ELLIPTICAL_PARTICLES = 30000; // pick your N

float M_ell = 400.0f; // thesis Fig 7.2 value
float a_ell = 4.0f;   // thesis Fig 7.2 value



float ell_mass_per_particle = M_ell / NUM_ELLIPTICAL_PARTICLES;

std::uniform_real_distribution<float> dis_uniform_ell(0.0f, 1.0f);

for (int i = NUM_DISK_PARTICLES+NUM_HALO_PARTICLES; i < NUM_PARTICLES; ++i)
{
    float u1 = dis_uniform_ell(gen);
    float u2 = dis_uniform_ell(gen);
    float u3 = dis_uniform_ell(gen);

    if (u1 >= 0.9999f)
        u1 = 0.9999f;

    float sqrt_u1 = std::sqrt(u1);
    float r = a_ell * (sqrt_u1 / (1.0f - sqrt_u1));

    // truncation radius scaled to a_ell, not a hardcoded constant
    if (r > 5.0f * a_ell)
    {
        i--;
        continue;
    ...
    particles[i].posMass = glm::vec4(x+15.0f, y+10.0f, z, ell_mass_per_particle);
    // Pure Hernquist potential -- no disk term, nothing to add it to
    float Phi_r = -(G * M_ell) / (r + a_ell);

    float v_esc = sqrt(2.0f * abs(Phi_r));
    float v_g = std::sqrt((G * M_ell) / a_ell);

    float v_mag = 0.0f;
    bool accepted = false;

    // Dynamic f_max search (eq. 6.9), identical method
    float f_max = 0.0f;
    for (int step = 1; step <= 500; step++)
    {
        float test_v = (v_esc * step) / 50.0f;
        ...
        float test_q = std::sqrt(-(a_ell * test_E) / (G * M_ell));
        float test_q2 = test_q * test_q;
        float q_safe = std::max(1.0f - test_q2, 0.0f);
        ...
        float denom = 8.0f * std::sqrt(2.0f) * std::pow(3.14159265f, 3.0f) * (a_ell * a_ell * a_ell) * (v_g * v_g * v_g) * std::pow(q_safe, 2.5f);
        float p_test = (test_v * test_v) * (M_ell / denom) * (t1 + t2);
    ...
    // Acceptance-Rejection Loop for speed, identical method
    int counter = 0;
    while (!accepted)
    {
        counter++;
        if (counter > 50000)
        {
            v_mag = dis_uniform_ell(gen) * v_esc * 0.5f;
            break;
        }

        float v_guess = dis_uniform_ell(gen) * v_esc;
        float u_prob = dis_uniform_ell(gen) * f_max;

        float E = 0.5f * v_guess * v_guess + Phi_r;
        if (E >= 0.0f)
            continue;

        float q = std::sqrt(-(a_ell * E) / (G * M_ell));
        float q2 = q * q;
        if (q < 0.0f) q = 0.0f;
        if (q > 0.9999f) q = 0.9999f;

        float term1 = 3.0f * std::asin(q);
        float term2 = q * std::sqrt(1.0f - q2) * (1.0f - 2.0f * q2) * (8.0f * q2 * q2 - 8.0f * q2 - 3.0f);

        float denominator = 8.0f * std::sqrt(2.0f) * std::pow(3.14159265f, 3.0f) * (a_ell * a_ell * a_ell) * (v_g * v_g * v_g) * std::pow(1.0f - q2, 2.5f);
        float f_E = (M_ell / denominator) * (term1 + term2);
        float P_target = (v_guess * v_guess) * f_E;
    ...
    // Isotropic direction -- this IS the final velocity, nothing added on top
    float cos_theta_v = 1.0f - 2.0f * dis_uniform_ell(gen);
    float sin_theta_v = std::sqrt(std::max(1.0f - cos_theta_v * cos_theta_v, 0.0f));
    float phi_v = 2.0f * 3.1415926535f * dis_uniform_ell(gen);

    float vx = v_mag * sin_theta_v * std::cos(phi_v);
    float vy = v_mag * sin_theta_v * std::sin(phi_v);
    float vz = v_mag * cos_theta_v;

    particles[i].velocity = glm::vec4(vx, vy, vz, 0.0f);
}
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glUniform1f(glGetUniformLocation(computeProgram, "M_halo"), M_halo);
        glUniform1f(glGetUniformLocation(computeProgram, "a_halo"), a_halo);
        glUniform1f(glGetUniformLocation(computeProgram, "M_disk"), M_disk);
        glUniform1f(glGetUniformLocation(computeProgram, "a_disk"), a_disk);
        glUniform1f(glGetUniformLocation(computeProgram, "b_disk"), b_disk);
        ...
        glUniform1i(glGetUniformLocation(renderProgram, "u_ellipticalStartIndex"), NUM_DISK_PARTICLES + NUM_HALO_PARTICLES);
