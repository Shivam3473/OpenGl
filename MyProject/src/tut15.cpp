// Camera : Move around /Mouse input
...
int main()
{
    ...
    glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED); // locks + hides cursor, here we need to add these so that the mopuse_callback is called and the cursor is also disabled
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::mat4 projection;
        projection = glm::perspective(glm::radians(fov), 800.0f / 600.0f, 0.1f, 100.0f);  // this would be based on the fov value is important feature to be noted
